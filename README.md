...
- **ポートフォワード**: WAN 側の UDP `RTP_PORT`〜`RTP_PORT+RTP_PORT_RANGE` をボットのホストへ転送する（HGW → ボット方向）。
- **ファイアウォール許可**: ボットのホストから HGW への UDP を許可する（ボット → HGW 方向）。この方向の宛先ポートは HGW 側の動的な RTP ポート（49xxx 番台など、毎回変わる）なので、宛先ポートで絞らず、送信元ポート `RTP_PORT`〜`RTP_PORT+RTP_PORT_RANGE` か送信元ホストで絞ります。

ボット → HGW 方向が塞がれていると、HGW からの音声は録音できるのにボットの再生音声が相手に届かない、という片方向の症状になります。
